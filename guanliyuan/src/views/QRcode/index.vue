  ...
  <div class="app-container">
    ...
    <el-table :data="tableData" border style="width: 100%" @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="name" label="二维码" width="95px">
        <template slot-scope="scope">
          <el-image
            ...
            :src="'api/stu/qrCode/'+scope.row.id+'.jpg'"
            @click="imgMax('/api/stu/qrCode/'+scope.row.id+'.jpg')"
          >
      ...
      <el-table-column prop="name" label="二维码id">
        <template slot-scope="scope">
          <span class="shenglue" :title="scope.row.id">{{ scope.row.id }}</span>
        </template>
      </el-table-column>
    ...
    <el-dialog title="生成二维码" style="min-width:700px" :visible.sync="dialogFormadd">
      <el-form :model="query">
        <el-row>
          <el-col class="mb" :span="9">
            <span class="mt must">生成数量：</span>
          </el-col>
          <el-col class="mb" :span="15">
            <el-input v-model="query.count" type="num" min="1" autocomplete="off" />
          </el-col>
          <el-col class="mb" :span="9">
            <span class="mt">绑定俱乐部：</span>
          </el-col>
          <el-col class="mb" :span="15">
          ...
          <el-col class="mb" :span="9">
            <span class="mt">下载生成的二维码：</span>
          </el-col>
          <el-col class="mb" :span="15">
            <el-select v-model="query.isDowbload" clearable style="width:100%" placeholder="请选择">
              <el-option
                v-for="item in options_sf"
                :key="item.name"
                :label="item.name"
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button :disabled="query.count<1" type="primary" @click="add()">确 定</el-button>
      </div>
    </el-dialog>
    ...
    <el-dialog title="选中的二维码分配给俱乐部" style="min-width:700px" :visible.sync="dialogFenpeiclub">
      <el-form :model="query_fenpei_club">
        <el-row>
          <el-col class="mb" :span="9">
            <span class="mt must">选择俱乐部：</span>
          </el-col>
          <el-col class="mb" :span="15">
            <el-select v-model="query_fenpei_club.clubId" style="width:100%" placeholder="请选择">
              <el-option
                v-for="item in clubs"
                :key="item.name"
                :label="item.name"
      ...
      <div slot="footer" class="dialog-footer">
        <el-button @click="dialogFenpeiclub = false">取 消</el-button>
        <el-button :disabled="!query_fenpei_club.clubId" type="primary" @click="add_fenpei_club()">确 定</el-button>
      </div>
    </el-dialog>
...
<script>
import { QRcodelist, QRcodeadd, QRcodearrange, QRcodeup, QRcodeClub } from '@/api/QRcode'
...
export default {
  ...
  methods: {
    ...
    fenpei_club() {
      for (const i in this.query_fenpei_club) {
        this.query_fenpei_club[i] = ''
      }
      this.dialogFenpeiclub = true
    },
    xiugais(res) {
      this.xiugai = true
      this.form = res
    ...
    add_fenpei_club() {
      ...
      if (this.query_fenpei_club.clubId) {
        form.clubId = this.query_fenpei_club.clubId
      }
      QRcodeClub(form).then(res => {
        if (res.num === 0) {
          this.$message.success('该二维码已被使用')
          this.getDate()
        } else if (res.num < str.length) {
          this.$message.success('部分二维码已被使用')
          this.getDate()
        } else {
          this.$message.success('绑定成功')
          this.getDate()
        }
        this.dialogFenpeiclub = false
      })
    },
    add() {
      const form = {}
      ...
      if (form.isDowbload) {
        let url = 'http://api.dev.lelunhua.com/langju-roller/qrcode/stu/generate' // 测试
        // let url = 'https://api.prod.lelunhua.com/langju-roller/qrcode/stu/generate' // 生产
        url += k
        window.open(url, '_blank')
      } else {
        QRcodeadd(form).then(res => {
        // this.$message.success('新增成功')
        })
      }
      const _this = this
      setTimeout(res => { _this.getDate() }, 2000)
      // this.getDate()
      this.dialogFormadd = false
    },
    arrange() {
      QRcodearrange(this.query).then(res => {
        this.$message.success('新增成功')
        this.getDate()
      })
      this.dialogFormadd = false
    ...
    down() {
      ...
      // axios.post('/api/qrcode/stu/download', ids)
      axios({
        method: 'post',
        url: '/api/sys/qrcode/stu/download',
        data: ids,
        responseType: 'arraybuffer'
      })
        .then(res => {
          if (res.data.byteLength < 2049) {
            const k = res.data
            var enc = new TextDecoder('utf-8')
            var val = JSON.parse(enc.decode(new Uint8Array(k))) // 转化成json对象
            this.$message.error(val.msg)
          } else {
            var blob = new Blob([res.data], { type: 'application/zip' }) // 指定格式为zip
        ...
        .catch(function(error) { // 请求失败处理
          // var enc = new TextDecoder('utf-8')
          // var val = JSON.parse(enc.decode(new Uint8Array(error.data))) // 转化成json对象
          // console.log(666)
          // console.log(val)
          console.log(error)
        })
